...
def main() -> int:
    ...
    if CONFIG["VERBOSE"]:
        print(
            "Found [{}] links for [{}]'s collection.".format(len(links), args.username)
        )
    if not links:
        print(
...
def get_download_links_for_user(_user: str) -> [str]:
    ...
    soup = BeautifulSoup(
        requests.get(USER_URL.format(_user), cookies=get_cookies()).text,
        "html.parser",
...
def download_album(_album_url: str, _attempt: int = 1) -> None:
    try:
        ...
        if not div:
            CONFIG["TQDM"].write(
                "ERROR: No div with pagedata found for album at url [{}]".format(
                    _album_url
                )
            )
            return
        ...
        if _attempt < CONFIG["MAX_URL_ATTEMPTS"]:
            if CONFIG["VERBOSE"] >= 2:
                CONFIG["TQDM"].write(
                    "WARN: HTTP Error on attempt # [{}] to download the album at [{}]. Trying again...".format(
                        _attempt, _album_url
                    )
                )
            ...
            print_exception(
                e,
                "An exception occurred trying to download album url [{}]:".format(
                    _album_url
                ),
            )
...
def download_file(_url: str, _to: str = None, _attempt: int = 1) -> None:
    try:
        with requests.get(
            ...
            if os.path.exists(file_path):
                if CONFIG["FORCE"]:
                    if CONFIG["VERBOSE"]:
                        CONFIG["TQDM"].write(
                            "--force flag was given. Overwriting existing file at [{}].".format(
                                file_path
                            )
                        )
                else:
                    ...
                    if expected_size == actual_size:
                        if CONFIG["VERBOSE"] >= 3:
                            CONFIG["TQDM"].write(
                                "Skipping album that already exists: [{}]".format(
                                    file_path
                                )
                            )
                        return
                        ...
                        if CONFIG["VERBOSE"] >= 2:
                            CONFIG["TQDM"].write(
                                "Album at [{}] is the wrong size. Expected [{}] but was [{}]. Re-downloading.".format(
                                    file_path, expected_size, actual_size
                                )
                            )

            if CONFIG["VERBOSE"] >= 2:
                CONFIG["TQDM"].write("Album being saved to [{}]".format(file_path))
        ...
        if _attempt < CONFIG["MAX_URL_ATTEMPTS"]:
            if CONFIG["VERBOSE"] >= 2:
                CONFIG["TQDM"].write(
                    "WARN: HTTP Error on attempt # [{}] to download the file at [{}]. Trying again...".format(
                        _attempt, _url
                    )
                )
            time.sleep(CONFIG["URL_RETRY_WAIT"])
            ...
            print_exception(
                e,
                "An exception occurred trying to download file url [{}]:".format(_url),
            )
    except Exception as e:
        print_exception(
...
def get_cookies():
    if CONFIG["BROWSER"] == "firefox":
        ...
        raise Exception(
            "Browser type if [{}] is unknown. Can't pull cookies, so can't authenticate with bandcamp.".format(
                CONFIG["BROWSER"]
            )
        )
